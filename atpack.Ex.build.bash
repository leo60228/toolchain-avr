...
for x in $ALL_DEVICE_SPECS; do
  DEFINITION=`cat ../objdir/lib/gcc/avr/${GCC_VERSION}/device-specs/${x} | grep -A1 cpp_mcu: | tail -n1 | cut -b 4-`
  if [ -z "$DEFINITION" ]; then
    DEFINITION=`cat ../objdir/lib/gcc/avr/${GCC_VERSION}/device-specs/${x} | grep __AVR_DEVICE_NAME__ | cut -f 1 -d " " | cut -b 4-`
  fi
  FANCY_NAME=`cat ../objdir/lib/gcc/avr/${GCC_VERSION}/device-specs/${x} | grep __AVR_DEVICE_NAME__ | cut -f2 -d"=" | cut -f1 -d" "`
  echo $DEFINITION
  echo $FANCY_NAME
